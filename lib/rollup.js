...
module.exports = function rollup(arr, discrete, cols) {
  ...
  console.log('ROLLUP start discrete=' + discrete);

  for (let i = 0; i < arr.length; i++) {
    if (arr[i].ts) arr[i].dtx = discrete ? transform(arr[i].ts, discrete) : arr[i].ts;
    // console.log(arr[i].dtx)
  }
  console.log('ROLLUP FIRST rec=' + util.inspect(arr[0]));
  console.log('ROLLUP LAST rec=' + util.inspect(arr[arr.length - 1]));
  ...
  */
  ...
  function calcOptime1() {
        ...
        if (index[dn]) {
          index[dn].forEach(idx => {
            ...
            if (vals[idx].val != curval) {
              ...
              if (idx == 1) {
                console.log(
                  'curval=' + curval + ' arr[j]=' + util.inspect(arr[j]) + ' vals[idx]' + util.inspect(vals[idx])
                );
              }
            }
          });
    ...
    console.log(curdtx + ' LAST getOneRow vals=' + util.inspect(vals));
    result.push(getOneRow('intervalSec'));
    return result;
