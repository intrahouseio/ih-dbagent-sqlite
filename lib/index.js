...
module.exports = async function(channel, opt, logger) {
  ...
  try {
    ...
    // NEW
    await client.createTable(getCreateTableStr('recipes'), 'recipes');

    channel.on('message', ({ id, type, query, payload, table }) => {
      if (type == 'write' && table == 'customtable') return writeCustom(id, payload, table);
      if (type == 'update') return updateCustom(query, payload, table); //[{id, $set:{field1:’newval’}}]
      ...
      for (j = 0; j < payload.length; j++) {
        ...
        const values = arr
          .map(
            i =>
              `'$.${i}', ${
                typeof payload[j]['$set'][i] === 'string' ? `'${payload[j]['$set'][i]}'` : payload[j]['$set'][i]
              }`
          )
          .join(', ');
  ...
  async function updateJson(query, payload, table) {
    ...
    for (j = 0; j < payload.length; j++) {
      ...
      const sql = `UPDATE ${table} SET ${json_field} = json_set(${json_field}, ${values}) WHERE ID = ${payload[j].id}`;
...
function getCreateTableStr(tableName) {
  ...
  switch (tableName) {
    ...
    case 'customtable':
      result =
        'id INTEGER PRIMARY KEY AUTOINCREMENT, ' + 'name TEXT NOT NULL, ' + 'ts INTEGER NOT NULL, ' + 'payload TEXT';
      break;
