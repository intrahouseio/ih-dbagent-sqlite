...
module.exports = async function(channel, opt, logger) {
  ...
  async function del(options) {
    ...
    for (const archDay of archDays) {
      let arrDnProp = options.rp.filter(function(object) {
        return object.days == archDay;
      });
        try {
          result = await client.deletePoints(tableName, archDay, arrDnProp, logger);
        } catch (err) {
          sendError(id, err);
        }
        logger.log(`Row(s) deleted ${result}`, 1);
...
function getCreateTableStr(tableName, fname) {
  ...
  return 'CREATE TABLE IF NOT EXISTS ' + fname + ' (' + result + ')';
}
