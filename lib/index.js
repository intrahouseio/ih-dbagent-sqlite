...
module.exports = async function(channel, opt, logger) {
  ...
  try {
    ...
    channel.on('message', ({ id, type, query, payload }) => {
      if (type == 'write') {
        if (overflow == 0) return write(id, payload);
  ...
  async function write(id, payload) {
    ...
    try {
      ...
      if (maxTimeWrite < endTime - beginTime) {
        ...
        if (process.connected) process.send({type:'procinfo', data:{lastMaxTimeWrite: maxTimeWrite, lastMaxCountWrite:payload.length}});
      }
