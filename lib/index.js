...
module.exports = async function (channel, opt, logger) {
  ...
  try {
    ...
    channel.on('message', ({ id, type, query, payload, table }) => {
      ...
      if (type == 'update')
        return table == 'customtable' ? updateCustom(query, payload, table) : update(query, payload, table); //[{id, $set:{field1:’newval’}}]
    ...
    if (query.sql) {
      ...
      for (j = 0; j < payload.length; j++) {
        ...
        const values = arr
          .map(
            i =>
              `'$.${i}', ${typeof payload[j]['$set'][i] === 'string' ? `'${payload[j]['$set'][i]}'` : payload[j]['$set'][i]
              }`
          )
          .join(', ');
